...
import config
from JaanX import LOGGER, app, userbot
from JaanX.core.call import AyushSolo
from JaanX.misc import sudo
from JaanX.plugins import ALL_MODULES
from JaanX.utils.database import get_banned_users, get_gbanned
...
async def init():
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("JaanX.plugins" + all_module)
    LOGGER("JaanX.plugins").info("Successfully Imported Modules...")
    await userbot.start()
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("JaanX").info("Stopping AnonX Music Bot...")
